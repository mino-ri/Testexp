...
type ArgGenBuilder internal () =
    member inline _.Bind(m: IArgumentGenerator<'T>, [<InlineIfLambda>] f: 'T -> ArgGenCode<'U>) =
        ArgGenCode<'U>(fun state -> (f (m.Generate(&state))).Invoke(&state))

    member inline _.Bind2(m1: IArgumentGenerator<'T1>, m2: IArgumentGenerator<'T2>, [<InlineIfLambda>] f: 'T1 * 'T2 -> ArgGenCode<'U>) =
    ...
    member inline _.BindReturn(m: IArgumentGenerator<'T>, [<InlineIfLambda>] f: 'T -> 'U) =
        ArgGenCode<'U>(fun state -> f (m.Generate(&state)))

    member inline _.BindReturn2(m1: IArgumentGenerator<'T1>, m2: IArgumentGenerator<'T2>, [<InlineIfLambda>] f: 'T1 * 'T2 -> 'U) =
        ArgGenCode<'U>(fun state ->
            let value1 = m1.Generate(&state)
            let value2 = m2.Generate(&state)
            f (value1, value2)
    ...
    member inline this.MergeSources(m1: IArgumentGenerator<'T1>, m2: IArgumentGenerator<'T2>) : ArgGenCode<'T1 * 'T2> =
    ...
    member inline _.Return(value: 'T) = ArgGenCode<'T>(fun _ -> value)

    member inline _.ReturnFrom(m: IArgumentGenerator<'T>) = ArgGenCode<'T>(fun state -> m.Generate(&state))

    member inline _.Zero() = ArgGenCode<unit>(fun _ -> ())

    member inline _.Delay([<InlineIfLambda>] f: unit -> ArgGenCode<'T>) = ArgGenCode<'T>(fun state -> (f ()).Invoke(&state))
    ...
    member inline _.For(source: seq<'T>, [<InlineIfLambda>] body: 'T -> ArgGenCode<unit>) : ArgGenCode<unit> =
        ArgGenCode<unit>(fun state ->
            for item in source do
                (body item).Invoke(&state)
        )
